...
- CI artifacts now keep the original plugin archive generated by `buildPlugin` and upload `build/distributions/*.zip` directly.
- Release workflow now uploads only the plugin ZIP to GitHub Releases (instead of all files under `build/distributions`).
- Added a release-time archive integrity check with `unzip -t build/distributions/*.zip` before uploading release assets.

### 📚 Documentation

- Added maintainer packaging guidance to README (EN/ZH), clarifying that Marketplace/manual installation must use `build/distributions/*.zip`.
- Documented a quick local pre-release verification flow:
  - `./gradlew buildPlugin`
...
[Unreleased]: https://github.com/zhongmiao-org/intercept-wave/compare/v4.0.1...HEAD
[4.0.1]: https://github.com/zhongmiao-org/intercept-wave/compare/v4.0.0...v4.0.1
