...
- 🔗 **访问地址**: 服务运行时的访问 URL
- **启动服务** / **停止服务**: 控制单个服务的启停
- **当前配置**: 显示端口、协议类型、上游地址，以及 HTTP Mock API 列表或 WS 推送规则等详细信息
...
WS 配置组与 HTTP 组共享基础配置（名称、端口、剥离前缀、启用），并提供以下 WS 专属选项：

- **上游 WebSocket 地址**: 上游 `ws://` 或 `wss://` 地址（例如：`ws://localhost:8080/ws/chat`），WS 组建议必填。
- **WS 前缀（可选）**: WebSocket 路径匹配前缀。当勾选“剥离前缀”且 WS 前缀非空时，会在匹配推送规则前剥离该前缀；为空时按原始路径匹配（不会继承 HTTP 拦截前缀）。
- **手动推送面板**: 控制工具窗口中是否显示 WS 手动推送区域。
- **WS 推送规则**: 用于自动推送与手动推送的规则表：
